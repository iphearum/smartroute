...
def test_existing_place_can_be_reused_as_shop_with_catalog_and_3d_asset(tmp_path):
    async def scenario():
        ...
        variant = await store.create_product_variant(
            product.id, sku="PALM-1KG", price="5.50", attributes={"size": "1 kg"},
        )
        inventory = await store.set_inventory(branch.id, variant.id, 20, 3)
        ...
        products = await store.list_products(business.id)
...
def test_inventory_rejects_variant_from_another_business(tmp_path):
    async def scenario():
        ...
        first = await store.create_business(display_name="First")
        second = await store.create_business(display_name="Second")
        branch = await store.create_shop_branch(first.id, place_id)
        product = await store.create_product(second.id, name="Other product")
        variant = await store.create_product_variant(product.id, sku="OTHER-1", price="1.00")
        with pytest.raises(ValueError, match="different business"):
            await store.set_inventory(branch.id, variant.id, 1)
        await store.close()
