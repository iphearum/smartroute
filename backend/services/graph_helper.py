...
class GraphHelper:
    def __init__(self, graph: dict[str, Any]):
        ...
        for edge in graph.get("links", []):
            ...
            if not isinstance(coordinates, (list, tuple)) or len(coordinates) < 2:
                ...
                first = self._haversine(source["x"], source["y"], *coordinates[0])
                last = self._haversine(source["x"], source["y"], *coordinates[-1])
                if last < first:
                    coordinates.reverse()
            line = LineString(coordinates)
            if line.is_empty or line.length == 0:
                continue
            self._edge_records.append((edge, line))
            edge_lines.append(line)
        self._edge_tree = STRtree(edge_lines) if edge_lines else None
    ...
    @lru_cache(maxsize=4096)
    def closest_edge(self, lat: float, lon: float):
        """Project a coordinate onto its nearest routable edge and return partial shapes."""
        if self._edge_tree is None:
            return None
        point = Point(lon, lat)
        index = int(self._edge_tree.nearest(point))
        edge, line = self._edge_records[index]
        position = line.project(point)
        projected = line.interpolate(position)
        left = substring(line, 0, position)
        right = substring(line, position, line.length)

        def coordinates(value, fallback):
            if isinstance(value, Point):
                return [fallback, [value.x, value.y]]
            return [list(item) for item in value.coords]

        left_coordinates = coordinates(left, [line.coords[0][0], line.coords[0][1]])
        right_coordinates = coordinates(right, [projected.x, projected.y])
        fraction = position / line.length
        edge_length = float(edge.get("length") or self._haversine(*line.coords[0], *line.coords[-1]))
        return {
            "source": edge["source"], "target": edge["target"],
            "point": [projected.x, projected.y],
            "snap_distance": self._haversine(lon, lat, projected.x, projected.y),
            "to_source_length": edge_length * fraction,
            "to_target_length": edge_length * (1 - fraction),
            "source_to_point": left_coordinates,
            "point_to_target": right_coordinates,
        }
