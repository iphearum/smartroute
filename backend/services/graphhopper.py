...
class GraphHopperClient:
    ...
    def route(
        ...
        routes = [self._normalize_path(path, rank) for rank, path in enumerate(paths[:limit], 1)]
        return {
            "routes": routes,
            "recommended_rank": 1,
    ...
    def route_combined(
        ...
        car_routes = self.route(coordinates, "car", 3, allow_destination_access)["routes"]
        options = [self._direct_suggestion(route, "car") for route in car_routes]
        try:
            motorcycle_routes = self.route(
                coordinates, "motorbike", 3, allow_destination_access
            )["routes"]
    ...
    @staticmethod
    def _normalize_path(path: dict[str, Any], rank: int):
        ...
        for instruction in instructions:
            ...
            steps.append({
                ...
                "duration": float(instruction.get("time") or 0) / 1000,
        ...
        return {
            ...
            "duration": float(path.get("time") or 0) / 1000,
