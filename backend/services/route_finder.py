...
class RouterEngine:
    ...
    def _clean_name(value):
        ...
        return None if value.casefold() in {"", "nan", "none", "null"} else value
    ...
    def get_path_segments(self, path):
        """Group verified OSM geometry separately from straight inferred edge shapes."""
        segments = []
        pairs = list(zip(path, path[1:]))
        for index, (u, v) in enumerate(pairs):
            edge = self.edges.get((u, v))
            name = str((edge or {}).get("name") or "").strip().casefold()
            highway = str((edge or {}).get("highway") or "").strip().casefold()
            unnamed_endpoint_spur = (
                index in {0, len(pairs) - 1}
                and name in {"", "nan", "none", "null"}
                and highway in {"residential", "service", "unclassified", "living_street", "road"}
            )
            verified = bool(
                edge and isinstance(edge.get("geometry"), (list, tuple))
                and not unnamed_endpoint_spur
            )
            if verified:
                coordinates = [list(point) for point in edge["geometry"]]
                if edge["source"] != u:
                    coordinates.reverse()
            else:
                source, target = self.nodes.get(u), self.nodes.get(v)
                if source is None or target is None:
                    continue
                coordinates = [[source["x"], source["y"]], [target["x"], target["y"]]]
            kind = "road" if verified else "inferred"
            if segments and segments[-1]["type"] == kind:
                if segments[-1]["geometry"][-1] == coordinates[0]:
                    coordinates = coordinates[1:]
                segments[-1]["geometry"].extend(coordinates)
            else:
                segments.append({"type": kind, "geometry": coordinates})
        return segments
