...
from services.map_models import (Business, CustomRoute, InventoryItem, MapImage,
                                 MapRecord, MapRevision, OsmFeature, Place, PlaceMedia,
                                 Product, ProductVariant, RoadClosure, ShopBranch,
                                 Storefront, ThreeDAsset)
...
class MapStore:
    ...
    async def create_business(self, **values):
        values["display_name"] = values["display_name"].strip()
        return await Business.create(**values)

    async def get_business(self, business_id: int):
        rows = await Business.filter(id=business_id).values(
    ...
    async def create_shop_branch(self, business_id: int, place_id: int, **values):
    ...
    async def create_storefront(self, business_id: int, **values):
        if not await Business.filter(id=business_id).exists():
            raise KeyError("Business not found")
        values["slug"] = self.validate_slug(values["slug"]).replace("_", "-")
        values["currency"] = values.get("currency", "USD").upper()
        return await Storefront.create(business_id=business_id, **values)

    async def create_product(self, business_id: int, **values):
        if not await Business.filter(id=business_id).exists():
            raise KeyError("Business not found")
        values["name"] = values["name"].strip()
        return await Product.create(business_id=business_id, **values)

    async def list_products(self, business_id: int):
        if not await Business.filter(id=business_id).exists():
            raise KeyError("Business not found")
        products = await Product.filter(business_id=business_id).order_by("id").values(
            "id", "name", "description", "category", "status", "metadata",
            "created_at", "updated_at",
        )
        for product in products:
            product["variants"] = await ProductVariant.filter(
                product_id=product["id"], active=True,
            ).order_by("id").values(
                "id", "sku", "title", "price", "compare_at_price", "attributes",
                "weight_grams", "active",
            )
        return products

    async def create_product_variant(self, product_id: int, **values):
        if not await Product.filter(id=product_id).exists():
    ...
    async def set_inventory(self, branch_id: int, variant_id: int,
                            quantity_available: int, quantity_reserved: int = 0):
