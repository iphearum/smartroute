...
// MapLibre resolves its worker script from import.meta.url of its own bundled
// chunk, which Next.js's bundler can't statically detect — the real worker
// file never gets emitted, so GeoJSON sources (route lines) silently never
// load. Self-hosting the worker module explicitly sidesteps that.
export const mapWorkerUrl = "/maplibre-gl/maplibre-gl-worker.mjs";
...
export function localBasemapCoversViewport(map: MapLibreMap) {
...
export function ensureComplexTextPlugin(maplibregl: typeof import("maplibre-gl")) {
