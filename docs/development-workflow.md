...
The right-side map action group provides current-location, browser-fullscreen, and reset-to-north/default-view actions. It uses the shared liquid surface and sits below the search and category stack on mobile, including the top safe-area inset. Current-location actions require HTTPS, use a 20-second high-accuracy request with a short-lived cached-position allowance, and place a persistent green `Your location` point with a reported-accuracy halo before centering the map; subsequent requests replace the previous location layers. Permission denial, unavailable Location Services, insecure context, and timeout failures must be shown to the user rather than silently ignored. Fullscreen remains a progressive enhancement because installed iOS PWAs already occupy their available standalone viewport and may not expose the browser Fullscreen API.
